      ...
      <div className="flex justify-center">
        ...
        <div className="flex gap-[200px] mt-[200px] max-[1330px]:flex-col max-[660px]:gap-[70px]">
          ...
          <div className="text-[32px] flex flex-col gap-2 max-[660px]:text-[24px]">
             ...
             <div className="flex justify-between ml-3"><p>Ulice:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("streat")}</p></div>
             <div className="flex justify-between ml-3"><p>Město:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("sity")}</p></div>
             <div className="flex justify-between ml-3"><p>Číslo popisné:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("psc")}</p></div>
             <div className="flex justify-between ml-3"><p>Země:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("county")}</p></div>
            </div>  
            <div className="text-[32px] flex flex-col gap-2 max-[660px]:text-[24px] ">
             <h2 className="text-[36px] text-white text-opacity-75 max-[660px]:text-[28px]">Osobní údaje</h2>
             <div className="flex justify-between ml-3"><p>Jméno:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("name")}</p></div>
             <div className="flex justify-between ml-3"><p>Příjmení:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("secondName")}</p></div>
             <div className="flex justify-between ml-3"><p>Email:</p> <p className="w-[250px] text-[22px] mt-[12px] max-[660px]:mt-[6px] max-[660px]:text-[18px]">{typeof window !== "undefined" && localStorage.getItem("email")}</p></div>
             <div className="flex justify-between ml-3"><p>Číslo:</p> <p className="w-[250px]">{typeof window !== "undefined" && localStorage.getItem("telefon")}</p></div>
            </div>
          </section>
          ...
          <section className="flex flex-col w-[550px] max-[660px]:w-[360px] max-[660px]:mx-auto ">
            ...
            <button className="py-[10px] px-[25px] w-auto self-start bg-orange-100 text-3xl max-[660px]:text-xl text-black font-bold ml-auto mt-auto">
              <Link href={{ pathname: "/", query: {} }}>Poslat objednavku</Link>
            </button>
          </section>
        </div>
      </div>
