...
export default function Header() {
  ...
  return (
    <header
      className={`relative fixed top-0 left-0 w-full z-50 bg-center transition-shadow duration-300 h-24 ${
        hasShadow ? "shadow-lg" : ""
      }`}
    ...
    >
      ...
      {/* Větší výška a posunutí dolů */}
      <div className="flex justify-between items-center p-4 container mx-auto">
        {/* Logo */}
        <Link href="/" className="text-white text-5xl">
          {" "}
          {/* Přidání bílého textu pro logo */}
          Jovana Šichová
        </Link>
        {/* Desktop menu */}
          ...
          {navLinks.map((link, i) => (
            <button key={i} onClick={() => setNavbarOpen(false)}>
              <Link
                key={i}
                href={link.href}
                className="text-[23px] font-medium text-white hover:text-blue-500 transition flex items-center gap-x-2"
              >
